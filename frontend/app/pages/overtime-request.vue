  ...
  <div>
    ...
    <div class="sticky top-0 z-20 border-b border-gray-100 bg-white px-6 pb-4 pt-12">
      <div class="mb-2 flex items-center gap-4">
        ...
        <h1 class="text-xl font-bold text-gray-800">Pengajuan Lembur</h1>
      </div>
    </div>
    ...
    <div class="px-4 py-4">
      ...
      <div class="mb-6 space-y-4">
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <label class="mb-2 block text-sm font-medium text-gray-600">Tanggal Lembur</label>
          <input v-model="form.date" type="date" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 transition focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/10" />
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div class="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <label class="mb-2 block text-sm font-medium text-gray-600">Jam Mulai</label>
            <input v-model="form.start_time" type="time" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 transition focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/10" />
          </div>
          <div class="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <label class="mb-2 block text-sm font-medium text-gray-600">Jam Selesai</label>
            <input v-model="form.end_time" type="time" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 transition focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/10" />
          </div>
        </div>
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        >
          ...
          {{ saving ? 'Mengirim…' : 'Kirim Pengajuan Lembur' }}
        </button>
      </div>

      <!-- Riwayat -->
      <div class="mt-8">
        <h3 class="mb-4 font-bold text-gray-800">Riwayat Lembur</h3>
        ...
        <div v-else class="space-y-3">
          <div v-for="r in requests" :key="r.id" class="flex items-center gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <div class="min-w-0 flex-1">
              <p class="text-sm font-medium text-gray-800">{{ formatDate(r.date) }} · {{ r.start_time }}–{{ r.end_time }}</p>
...
async function loadRequests() {
  ...
  try {
    const data = await api<{ data: any[] }>('GET', '/overtime-requests/me')
...
async function submit() {
  ...
  if (!form.date || !form.start_time || !form.end_time || !form.reason.trim()) {
    error.value = 'Lengkapi tanggal, jam, dan alasan dulu ya.'
    return
  }
  saving.value = true
  try {
    ...
    error.value = errorMessage(e, 'Gagal mengirim pengajuan lembur.')
  } finally {
    saving.value = false
  }
}
...
async function cancel(r: any) {
  if (!confirm(`Batalkan pengajuan lembur ${formatDate(r.date)}?`)) return
  try {
    await api('POST', `/overtime-requests/${r.id}/cancel`)
    toast.success('Pengajuan lembur dibatalkan.')
    await loadRequests()
  } catch (e: any) {
    toast.error(errorMessage(e, 'Gagal membatalkan pengajuan.'))
  }
}
